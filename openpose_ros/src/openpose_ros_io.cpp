...
OpenPoseROSIO::OpenPoseROSIO(OpenPose &openPose): nh_("/openpose_ros_node"), it_(nh_)
{
    ...
    std::string poses_topic_Nose;
    std::string poses_topic_LEye;
    std::string poses_topic_RElbow;
    std::string poses_topic_RWrist;
    std::string poses_topic_Fingertip;
    std::string poses_topic_First_joint;
    std::string poses_topic_Second_joint;
    std::string poses_topic_Third_joint;
    std::string input_image_transport_type;

    nh_.param("poses_topic_Nose", poses_topic_Nose, std::string("/openpose_ros/poses_Nose"));
    nh_.param("poses_topic_LEye", poses_topic_LEye, std::string("/openpose_ros/poses_LEye"));
    nh_.param("poses_topic_RElbow", poses_topic_RElbow, std::string("/openpose_ros/poses_RElbow"));
    nh_.param("poses_topic_RWrist", poses_topic_RWrist, std::string("/openpose_ros/poses_RWrist"));
    nh_.param("poses_topic_Fingertip", poses_topic_Fingertip, std::string("/openpose_ros/poses_Fingertip"));
    nh_.param("poses_topic_First_joint", poses_topic_First_joint, std::string("/openpose_ros/poses_First_joint"));
    nh_.param("poses_topic_Second_joint", poses_topic_Second_joint, std::string("/openpose_ros/poses_Second_joint"));
    nh_.param("poses_topic_Third_joint", poses_topic_Third_joint, std::string("/openpose_ros/poses_Third_joint"));
    ...
    poses_pub_Nose = nh_.advertise<openpose_ros_msgs::PointWithProb>(poses_topic_Nose, 10);
    poses_pub_LEye = nh_.advertise<openpose_ros_msgs::PointWithProb>(poses_topic_LEye, 10);
    poses_pub_RElbow = nh_.advertise<openpose_ros_msgs::PointWithProb>(poses_topic_RElbow, 10);
    poses_pub_RWrist = nh_.advertise<openpose_ros_msgs::PointWithProb>(poses_topic_RWrist, 10);
    poses_pub_Fingertip = nh_.advertise<openpose_ros_msgs::PointWithProb>(poses_topic_Fingertip, 10);
    poses_pub_First_joint = nh_.advertise<openpose_ros_msgs::PointWithProb>(poses_topic_First_joint, 10);
    poses_pub_Second_joint = nh_.advertise<openpose_ros_msgs::PointWithProb>(poses_topic_Second_joint, 10);
    poses_pub_Third_joint = nh_.advertise<openpose_ros_msgs::PointWithProb>(poses_topic_Third_joint, 10);
...
void OpenPoseROSIO::printKeypoints(const std::shared_ptr<std::vector<std::shared_ptr<op::Datum>>>& datumsPtr)
{
    ...
    {
        //op::opLog("\nKeypoints:");
        openpose_ros_msgs::PointWithProb msgs_poses_Nose;
        openpose_ros_msgs::PointWithProb msgs_poses_LEye;
        openpose_ros_msgs::PointWithProb msgs_poses_RElbow;
        openpose_ros_msgs::PointWithProb msgs_poses_RWrist;
          ...
          // op::opLog("Person " + std::to_string(person) + " (x, y, score):");
            for (auto bodyPart = 0 ; bodyPart < poseKeypoints.getSize(1) ; bodyPart++)
            {
                ...
                for (auto xyscore = 0 ; xyscore < poseKeypoints.getSize(2) ; xyscore++)
                {
                  ...
                  if(bodyPart==14){//右目：14、左目：15、鼻：0
                    msgs_poses_Nose.x=poseKeypoints[{0,bodyPart,0}];
                    msgs_poses_Nose.y=poseKeypoints[{0,bodyPart,1}];
                    msgs_poses_Nose.prob=poseKeypoints[{0,bodyPart,2}];
                    // std::cout << msgs_poses_Nose.x<< msgs_poses_Nose.y;
                  }
                  if(bodyPart==15){
                    msgs_poses_LEye.x=poseKeypoints[{0,bodyPart,0}];
                    msgs_poses_LEye.y=poseKeypoints[{0,bodyPart,1}];
                    msgs_poses_LEye.prob=poseKeypoints[{0,bodyPart,2}];
                    // std::cout << msgs_poses_LEye.x<< msgs_poses_LEye.y;
                  }
                  if(bodyPart==3){
                    msgs_poses_RElbow.x=poseKeypoints[{0,bodyPart,0}];
                    msgs_poses_RElbow.y=poseKeypoints[{0,bodyPart,1}];
                    msgs_poses_RElbow.prob=poseKeypoints[{0,bodyPart,2}];
                }
                  if(bodyPart==4){
                    msgs_poses_RWrist.x=poseKeypoints[{0,bodyPart,0}];
                    msgs_poses_RWrist.y=poseKeypoints[{0,bodyPart,1}];
                    msgs_poses_RWrist.prob=poseKeypoints[{0,bodyPart,2}];
                  }
                }
              //  op::opLog(valueToPrint);
                poses_pub_Nose.publish(msgs_poses_Nose);
                poses_pub_LEye.publish(msgs_poses_LEye);
                poses_pub_RElbow.publish(msgs_poses_RElbow);
                poses_pub_RWrist.publish(msgs_poses_RWrist);
            }
        }
        op::opLog(" ");
    ...
    if (datumsPtr != nullptr && !datumsPtr->empty())
    {
        ...
        for (auto person = 0 ; person < poseKeypoints.getSize(0) ; person++)
        {
            ...
            if(FLAGS_hand)
            {
                ...
                openpose_ros_msgs::PointWithProb msgs_poses_Fingertip;
                openpose_ros_msgs::PointWithProb msgs_poses_First_joint;
                openpose_ros_msgs::PointWithProb msgs_poses_Second_joint;
                openpose_ros_msgs::PointWithProb msgs_poses_Third_joint;

                // std::cout<<"rightHandKeypoints.getSize(1):"<<rightHandKeypoints.getSize(1)<<" ";
                //
                for (auto handPart = 0 ; handPart < rightHandKeypoints.getSize(1) ; handPart++)
                {
                    ...
                    if(handPart==5){
                      msgs_poses_Third_joint.x=rightHandKeypoints[{0,handPart,0}];
                      msgs_poses_Third_joint.y=rightHandKeypoints[{0,handPart,1}];
                      msgs_poses_Third_joint.prob=rightHandKeypoints[{0,handPart,2}];
                      // std::cout<<"5_x:"<<rightHandKeypoints[{0,handPart,0}]<<" "<<"5_y:"<<rightHandKeypoints[{0,handPart,1}]<<" ";
                    }

                    if(handPart==6){
                      msgs_poses_Second_joint.x=rightHandKeypoints[{0,handPart,0}];
                      msgs_poses_Second_joint.y=rightHandKeypoints[{0,handPart,1}];
                      msgs_poses_Second_joint.prob=rightHandKeypoints[{0,handPart,2}];
                      // std::cout<<"6_x:"<<rightHandKeypoints[{0,handPart,0}]<<" "<<"6_y:"<<rightHandKeypoints[{0,handPart,1}]<<" ";
                    }

                    if(handPart==7){
                      msgs_poses_First_joint.x=rightHandKeypoints[{0,handPart,0}];
                      msgs_poses_First_joint.y=rightHandKeypoints[{0,handPart,1}];
                      msgs_poses_First_joint.prob=rightHandKeypoints[{0,handPart,2}];
                      // std::cout<<"7_x:"<<rightHandKeypoints[{0,handPart,0}]<<" "<<"7_y:"<<rightHandKeypoints[{0,handPart,1}]<<" ";
                    }

                    if(handPart==8){
                      msgs_poses_Fingertip.x=rightHandKeypoints[{0,handPart,0}];
                      msgs_poses_Fingertip.y=rightHandKeypoints[{0,handPart,1}];
                      msgs_poses_Fingertip.prob=rightHandKeypoints[{0,handPart,2}];
                      // std::cout<<"8_x:"<<rightHandKeypoints[{0,handPart,0}]<<" "<<"8_y:"<<rightHandKeypoints[{0,handPart,1}]<<" ";
                    }
                ...
                poses_pub_Fingertip.publish(msgs_poses_Fingertip);
                poses_pub_First_joint.publish(msgs_poses_First_joint);
                poses_pub_Second_joint.publish(msgs_poses_Second_joint);
                poses_pub_Third_joint.publish(msgs_poses_Third_joint);
